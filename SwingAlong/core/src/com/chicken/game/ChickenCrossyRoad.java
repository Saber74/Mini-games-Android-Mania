...
public class ChickenCrossyRoad extends ApplicationAdapter implements InputProcessor{
	...
	//title page textures
	Texture title;
	boolean game = true;//to tell whether or not program is running game or on a title page
	boolean start = false;
	...
	public void render () {
		...
		//RUN THE GAME
		if(players[0].isAlive() && game || players[1].isAlive() && game){
	...
	public void overlapHopOn(Player p, Rectangle hRect){
	...
	public void nestOverlap(Player p){
		for(int i=0; i<homes.length; i++){
