...
public class ChickenCrossyRoad extends ScreenAdapter {
	...
	//title page textures
	Texture title;
	boolean play = false;//to tell whether or not program is running game or on a title page
	...
	public ChickenCrossyRoad(MyGdxGame game) {
		...
		//music = Gdx.audio.newMusic(Gdx.files.internal("ChickenCrossyRoad/chicken_dance_song.mp3"));
	...
	public void render(float delta) {
		...
		if (players[0].won() || !players[1].isAlive()) {
			...
			font.draw(batch, "Player 2 Score: " + players[1].score, 280, 550);
			batch.end();
		}

		//PLAYER 2 WINS GAME!
		else if (players[1].won() || !players[0].isAlive()) {
			play = false;
			batch.begin();
			...
			batch.begin();
			font.draw(batch, "First player to get 3 chickens to the end", 160, 650);
			font.draw(batch, "OR",380, 600);
			font.draw(batch, "Last player standing",280, 550);
			font.draw(batch, "WINS!",360, 480);
			font.draw(batch, "Press <SPACE> to begin",230, 350);
			batch.end();
		}

		//RUN THE GAME
		if (play && players[0].isAlive() && play || players[1].isAlive() && play) {
